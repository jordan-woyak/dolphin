...
ATTR_TARGET DOLPHIN_FORCE_INLINE static void LoadTransposedPosYMM(const void* source, __m256& o0,
    __m256& o1, __m256& o2, __m256& o3)
{
  const Vector* vsource = static_cast<const Vector*>(source);
  o0 = _mm256_broadcast_ps(&vsource[0]);
...
ATTR_TARGET static void TransformVertices(void* output, const void* vertices, u32 stride, int count)
{
  ...
  for (int i = 1; i < count; i += 2)
  {
    ...
    __m256 v01 = LoadTransform2Vertices<PositionHas3Elems, PerVertexPosMtx>(v0data, v1data, pos0,
        pos1, pos2, pos3, proj0, proj1, proj2, proj3);
  ...
  {
    *voutput = LoadTransformVertex<PositionHas3Elems, PerVertexPosMtx>(cvertices, pos0, pos1, pos2,
        pos3, proj0, proj1, proj2, proj3);
    cvertices += stride;
    voutput += 1;
  }
#endif

...
void PreprocessIndexedXF(CPArray array, u32 index, u16 address, u8 size)
{
  ...
  auto& memory = system.GetMemory();
  const u8* new_data =
      memory.GetPointerForRange(g_preprocess_cp_state.array_bases[array] +
...
#define DescriptionlessReg(reg) std::make_pair(RegName(reg), "");
  ...
  case XFMEM_SETTEXMTXINFO + 6:
  case XFMEM_SETTEXMTXINFO + 7:
    return std::make_pair(fmt::format("XFMEM_SETTEXMTXINFO Matrix {}",
                              address - XFMEM_SETTEXMTXINFO),
  ...
  case XFMEM_SETPOSTMTXINFO + 6:
  case XFMEM_SETPOSTMTXINFO + 7:
    return std::make_pair(fmt::format("XFMEM_SETPOSTMTXINFO Matrix {}",

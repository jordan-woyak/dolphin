    ...
    const TextureInfo& texture_info)
{
  ...
  {
    auto decoded_entry = ApplyPaletteToEntry(unconverted_copy->second,
...
std::unique_ptr<AbstractStagingTexture> TextureCacheBase::GetEFBCopyStagingTexture()
{
  ...
  std::unique_ptr<AbstractStagingTexture> tex =
      g_gfx->CreateStagingTexture(StagingTextureType::Readback,
          m_efb_encoding_texture->GetConfig());
  if (!tex)
    ...
    const std::array<u32, 3>& filter_coefficients)
{
  ...
  const auto framebuffer_rect = g_gfx->ConvertFramebufferRectangle(scaled_src_rect,
    ...
    u32 aligned_height, u32 row_stride, const u8* palette, TLUTFormat palette_format)
{
  ...
  const AbstractShader* shader = g_shader_cache->GetTextureDecodingShader(format,
      info->palette_size != 0 ? std::make_optional(palette_format) : std::nullopt);
  if (!shader)
    return false;

...
bool ParsePropertyValue(const CustomAssetLibrary::AssetID& asset_id,
    const picojson::value& json_value, std::string_view type, MaterialProperty::Value* value)
{
  if (type == "int")
  {
...
bool ParseMaterialProperties(const CustomAssetLibrary::AssetID& asset_id,
    const picojson::array& values_data, std::vector<MaterialProperty>* material_property)
{
...
void MaterialProperty::WriteToMemory(u8*& buffer, const MaterialProperty& property)
{
  ...
  std::visit(
      overloaded{[&](s32 value) { write_memory(&value, sizeof(s32)); },
...
{
  std::size_t result = 0;
  std::visit(overloaded{[&](s32 value) { result = MemorySize; }, [&](const std::array<s32, 2>&)
                 { result = MemorySize; }, [&](const std::array<s32, 3>&) { result = MemorySize; },
                 [&](const std::array<s32, 4>&) { result = MemorySize; },
                 [&](float) { result = MemorySize; }, [&](const std::array<float, 2>&)
                 { result = MemorySize; }, [&](const std::array<float, 3>&)
                 { result = MemorySize; }, [&](const std::array<float, 4>&)
                 { result = MemorySize; }, [&](bool) { result = MemorySize; }},
      property.m_value);
    ...
    const picojson::object& json, MaterialData* data)
{
  ...
  {
    ERROR_LOG_FMT(VIDEO,
        "Asset '{}' failed to parse json, 'next_material_asset' not found or wrong type", asset_id);
    return false;
  }

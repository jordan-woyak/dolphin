...
Common::Matrix44 BuildMatrixFromNode(const tinygltf::Node& node)
{
  ...
  if (!node.scale.empty())
  {
    matrix *= Common::Matrix44::FromMatrix33(Common::Matrix33::Scale(Common::Vec3{
  ...
  {
    matrix *=
        Common::Matrix44::FromQuaternion(Common::Quaternion(static_cast<float>(node.rotation[3]),

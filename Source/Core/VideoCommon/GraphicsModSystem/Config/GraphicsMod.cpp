...
bool GraphicsModConfig::DeserializeFromConfig(const picojson::value& value)
{
  ...
  if (groups.is<picojson::array>())
  {
    for (const auto& group_val : groups.get<picojson::array>())
    {
      ...
      {
        ERROR_LOG_FMT(VIDEO,
            "Failed to load mod configuration file, specified group is not a json object");
        return false;
      }
  ...
  if (features.is<picojson::array>())
  {
    for (const auto& feature_val : features.get<picojson::array>())
    {
      ...
      {
        ERROR_LOG_FMT(VIDEO,
            "Failed to load mod configuration file, specified feature is not a json object");
        return false;
      }
  ...
  if (assets.is<picojson::array>())
  {
    for (const auto& asset_val : assets.get<picojson::array>())
    {
      ...
      {
        ERROR_LOG_FMT(VIDEO,
            "Failed to load mod configuration file, specified asset is not a json object");
        return false;
      }

...
bool FramebufferManager::CreateEFBFramebuffer()
{
  ...
  if (g_ActiveConfig.MultisamplingEnabled())
  {
    ...
    m_efb_resolve_color_texture = g_gfx->CreateTexture(TextureConfig(efb_color_texture_config.width,
                                                           efb_color_texture_config.height, 1,
                                                           efb_color_texture_config.layers, 1,
                                                           efb_color_texture_config.format, flags,
...
void FramebufferManager::PopulateEFBCache(bool depth, u32 tile_index, bool async)
{
  ...
  const bool force_intermediate_copy =
      depth &&
      (!g_backend_info.bSupportsDepthReadback ||
  ...
  if (GetEFBScale() != 1 || force_intermediate_copy)
  {
    ...
    g_gfx->SetSamplerState(0,
        depth ? RenderState::GetPointSamplerState() : RenderState::GetLinearSamplerState());
    g_gfx->Draw(0, 3);
...
bool FramebufferManager::CompilePokePipelines()
{
  ...
  config.rasterization_state =
      RenderState::GetNoCullRasterizationState(g_backend_info.bSupportsLargePoints ?
...
void FramebufferManager::DoLoadState(PointerWrap& p)
{
  ...
  g_gfx->SetSamplerState(0,
      rescale ? RenderState::GetLinearSamplerState() : RenderState::GetPointSamplerState());

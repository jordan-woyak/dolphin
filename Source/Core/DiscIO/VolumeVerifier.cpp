      ...
      [this, platform]() -> std::vector<PotentialMatch>
      {
        ...
        case DownloadStatus::FailButOldCacheAvailable:
          ERROR_LOG_FMT(DISCIO,
              "Failed to fetch data from Redump.org, using old cached data instead");
          [[fallthrough]];
  ...
  if (content_read)
  {
    m_content_future = std::async(std::launch::async,
        [this, read_failed, content]
        {
          ...
          {
            AddProblem(Severity::High,
                Common::FmtFormatT("Content {0:08x} is corrupt.", content.id));
          }
        });
    ...
    m_group_future = std::async(std::launch::async,
        [this, read_failed, group_index = m_group_index]
        {
              ...
              ++block_index, offset_in_group += VolumeWii::BLOCK_TOTAL_SIZE)
          {
            ...
            if (!read_failed && m_volume.CheckBlockIntegrity(block_index,
                                    m_data.data() + offset_in_group, group.partition))
            {
            ...
            else
            {
              ...
              {
                WARN_LOG_FMT(DISCIO, "Integrity check failed for unused block at {:#x}",
                    block_offset);
...
void VolumeVerifier::Finish()
{
  ...
  for (auto [partition, blocks] : m_unused_block_errors)
  {
    if (blocks > 0)
    {
      const std::string name = GetPartitionName(m_volume.GetPartitionType(partition));
      AddProblem(Severity::Low,
          Common::FmtFormatT("Errors were found in {0} unused blocks in the {1} partition.", blocks,
              name));
    }
  }

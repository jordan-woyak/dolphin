    ...
    for (u32 i = 0; i < number_of_partitions; i++)
    {
      ...
      auto get_tmd = [this, partition]() -> IOS::ES::TMDReader
      {
        ...
        const std::optional<u64> tmd_address =
            ReadSwappedAndShifted(partition.offset + WII_PARTITION_TMD_OFFSET_ADDRESS,
                PARTITION_NONE);
      ...
      auto get_cert_chain = [this, partition]() -> std::vector<u8>
      {
        ...
        const std::optional<u64> address =
            ReadSwappedAndShifted(partition.offset + WII_PARTITION_CERT_CHAIN_OFFSET_ADDRESS,
                PARTITION_NONE);
        if (!size || !address)
          return {};
      ...
      auto get_h3_table = [this, partition]() -> std::vector<u8>
      {
        ...
        const std::optional<u64> h3_table_offset =
            ReadSwappedAndShifted(partition.offset + WII_PARTITION_H3_OFFSET_ADDRESS,
                PARTITION_NONE);
        if (!h3_table_offset)
          return {};
      ...
      m_partitions.emplace(partition,
          PartitionDetails{Common::Lazy<std::unique_ptr<Common::AES::Context>>(get_key),

...
void DSPManager::RegisterMMIO(MMIO::Mapping* mmio, u32 base)
{
  ...
  mmio->Register(base | DSP_CONTROL,
      ...
      MMIO::ComplexWrite<u16>(
          [](Core::System& system, u32, u16 val)
          {
            ...
            {
              PanicAlertFmt("DSPInterface (w) DSP state (CC00500A) gets a value with "
                            "junk in the padding {:08x}",
                  val);
            }
  ...
  mmio->Register(base | AUDIO_DMA_BLOCKS_LEFT,
      MMIO::ComplexRead<u16>(
          [](Core::System& system, u32)
          {
            // remaining_blocks_count is zero-based.  DreamMix World Fighters will hang
            // if it never reaches zero.
            auto& dsp = system.GetDSP();

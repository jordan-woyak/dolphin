            ...
            if (m_control.TSTART)
            {
              ...
              if (m_control.DMA == 0)
              {
                ...
                case EXI_READWRITE:
                  ...
                  break;
                default:
                  DEBUG_ASSERT_MSG(EXPANSIONINTERFACE, 0, "EXI Imm: Unknown transfer type {}",
                      m_control.RW);
                }
              }
              else
              {
                ...
                case EXI_WRITE:
                  ...
                  break;
                default:
                  DEBUG_ASSERT_MSG(EXPANSIONINTERFACE, 0, "EXI DMA: Unknown transfer type {}",
                      m_control.RW);
                }
              }
...
void CEXIChannel::DoState(PointerWrap& p)
{
  ...
  for (int device_index = 0; device_index < NUM_DEVICES; ++device_index)
  {
        ...
        !m_system.GetMovie().IsMovieActive())
    {
      ...
      m_system.GetExpansionInterface().ChangeDevice(m_channel_id, device_index,
          EXIDeviceType::MemoryCardFolder, CoreTiming::FromThread::CPU);
    }
  }
}

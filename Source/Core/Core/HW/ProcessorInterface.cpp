      ...
      MMIO::ComplexWrite<u32>(
          [](Core::System& system, u32, u32 val)
          {
            ...
            if ((val & 1) != 0)
            {
              ...
              // resets various pointers used by the video thread, we can't call it
              // directly from the CPU thread, so queue a task to do it instead. In
              // single-core mode, AsyncRequests is in passthrough mode, so this will be
              // safely and immediately called on the CPU thread.

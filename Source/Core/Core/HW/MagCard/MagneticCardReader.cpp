...
void MagneticCardReader::Process()
{
  while (true)
  {
    const auto read = GetInputSpan();
    if (read.empty())
      break;
...
void MagneticCardReader::BuildPacket()
{
  ...
  OutputByte(START_OF_TEXT);  // Not included in the checksum.

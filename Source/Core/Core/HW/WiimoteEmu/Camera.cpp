...
CameraLogic::GetCameraPoints(const Common::Matrix44& transform, Common::Vec2 field_of_view)
{
  ...
  std::ranges::transform(leds, camera_points.begin(),
      [&](const Vec3& v)
      {
        ...
        // FYI: truncating vs. rounding seems to produce more symmetrical cursor
        // positioning.
        ...
        // Curve fit from data using an official WM+ and sensor bar at middle
        // "sensitivity". Point sizes at minimum and maximum sensitivity differ
        // by around 0.5 (not implemented).

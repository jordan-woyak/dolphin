...
public:
  ...
  virtual Extension* GetActiveExtension() const = 0;
  // Active extension number is exposed for TAS.
  virtual ExtensionNumber GetActiveExtensionNumber() const = 0;
  virtual ControllerEmu::SubscribableSettingValue<int>& GetAttachmentSetting() = 0;
  virtual ControllerEmu::SubscribableSettingValue<bool>& GetMotionPlusSetting() = 0;
...
protected:
  ...
  virtual Common::Vec3 GetTotalAcceleration() const = 0;
  ...
  virtual void HandleIRLogicEnable2(const WiimoteCommon::OutputReportEnableFeature&) = 0;
  virtual void HandleSpeakerMute(const WiimoteCommon::OutputReportEnableFeature&) = 0;
  virtual void HandleSpeakerEnable(const WiimoteCommon::OutputReportEnableFeature&) = 0;
...
public:
  ...
  Extension* GetActiveExtension() const override;
  ...
  virtual ControllerEmu::SubscribableSettingValue<int>& GetAttachmentSetting() override;
  virtual ControllerEmu::SubscribableSettingValue<bool>& GetMotionPlusSetting() override;
...
protected:
  ...
  Common::Vec3 GetTotalAngularVelocity() const override;
...
public:
  ...
  Extension* GetActiveExtension() const override
  {
    // XXX const_cast seems wrong here, but how does Wiimote's version work then?
    return const_cast<Extension*>(static_cast<const Extension*>(&m_ext));
  }
  ...
  virtual ControllerEmu::SubscribableSettingValue<bool>& GetMotionPlusSetting() override
  {
    return m_fake_motion_plus_setting;
  }

  virtual ControllerEmu::SubscribableSettingValue<int>& GetAttachmentSetting() override
  {
    return m_fake_attachment_setting;
  }

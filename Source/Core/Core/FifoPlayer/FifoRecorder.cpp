    ...
    u8 vat, u32 vertex_size, u16 num_vertices, const u8* vertex_data)
{
  ...
  const u32 pos_direct_size = VertexLoader_Position::GetSize(VertexComponentFormat::Direct,
      vtx_attr.g0.PosFormat, vtx_attr.g0.PosElements);
  ...
  for (u32 i = 0; i < vtx_desc.high.TexCoord.Size(); i++)
  {
    ...
    const u32 tc_direct_size = VertexLoader_TextCoord::GetSize(VertexComponentFormat::Direct,
        vtx_attr.GetTexFormat(i), vtx_attr.GetTexElements(i));

...
std::optional<std::string> GetStringDescriptor(libusb_device_handle* dev_handle, uint8_t desc_index)
{
  ...
  const auto lang_id_result = libusb_get_string_descriptor(dev_handle, desc_index, 0,
  ...
  const auto str_result = libusb_get_string_descriptor(dev_handle, desc_index, lang_id,
      reinterpret_cast<unsigned char*>(&buffer), 255);

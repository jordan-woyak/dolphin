...
void AchievementManager::FetchBoardInfo(AchievementId leaderboard_id)
{
  ...
  rc_client_begin_fetch_leaderboard_entries_around_user(m_client, leaderboard_id, 4,
      LeaderboardEntriesCallback, callback_data_2);
}
...
void AchievementManager::DisplayWelcomeMessage()
{
  ...
  {
    OSD::AddMessage(fmt::format("{} achievements unsupported",
                        summary.num_unsupported_achievements),
        OSD::Duration::VERY_LONG, OSD::Color::RED);
  }
  OSD::AddMessage(fmt::format("Hardcore mode is {}",

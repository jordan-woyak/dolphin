...
NWC24::ErrorCode NetKDRequestDevice::KDCheckMail(u32* mail_flag, u32* interval)
{
  ...
  const std::string form_data(fmt::format("mlchkid={}&chlng={}", m_config.GetMlchkid(),
      random_number));
...
NWC24::ErrorCode NetKDRequestDevice::KDSendMail()
{
  ...
  for (const u32 file_index : mails)
  {
    ...
    NWC24::ErrorCode res = NWC24::ReadFromVFF(NWC24::Mail::SEND_BOX_PATH,
        m_send_list.GetMailPath(file_index), m_ios.GetFS(), mail_data);
    if (res != NWC24::WC24_OK)
    {
  ...
  Common::ScopeGuard state_guard(
      [&]
      {
        ...
        if (success)
        {
          ...
          m_dl_list.SetNextDownloadTime(entry_index,
              current_utc + m_dl_list.GetDownloadMargin(entry_index), subtask_id);
        }
        else
        {
          // Else set it to the retry margin
          m_dl_list.SetNextDownloadTime(entry_index,
              current_utc + m_dl_list.GetRetryTime(entry_index), subtask_id);
        }

...
void MicrophoneLogitech::StreamStart(u32 sampling_rate)
{
  ...
  m_worker.Execute([this, sampling_rate] {
    ...
    cubeb_devid input_device =
        CubebUtils::GetInputDeviceById(Config::Get(Config::MAIN_LOGITECH_MIC_1_MICROPHONE));
    if (cubeb_stream_init(m_cubeb_ctx.get(), &m_cubeb_stream, "Dolphin Emulated Logitech Mic 1",
...
long MicrophoneLogitech::CubebDataCallback(cubeb_stream* stream, void* user_data,
                                           ...
                                           long nframes)
{
  ...
  if (Config::Get(Config::MAIN_LOGITECH_MIC_1_MUTED))
    return nframes;
...
long MicrophoneLogitech::DataCallback(const SampleType* input_buffer, long nframes)
{
  ...
  // Skip data if the mute is on.
  if (m_sampler.mute)
    return nframes;

  std::span<const SampleType> buffer(input_buffer, nframes);
  const auto gain = ComputeGain(Config::Get(Config::MAIN_LOGITECH_MIC_1_VOLUME_MODIFIER));
  ...
  for (const SampleType le_sample : std::ranges::transform_view(buffer, apply_gain))
  {
    ...
    m_stream_buffer[m_stream_wpos] = le_sample;
...
{
  if (m_sampler.mute || Config::Get(Config::MAIN_LOGITECH_MIC_1_MUTED))
    return 0;
...
const MicrophoneLogitech::FloatType MicrophoneLogitech::Loudness::DB_MAX =
    20 * std::log10(FloatType(1));

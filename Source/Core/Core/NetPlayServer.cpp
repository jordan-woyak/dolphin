    ...
    const u8 channel_id)
{
  {
    std::lock_guard lkq(m_crit.async_queue_write);
    m_async_queue.Push(AsyncQueueEntry{
        std::move(packet), skip_pid, TargetMode::AllExcept, channel_id});
  }
  Common::ENet::WakeupThread(m_server);
}
...
{
  {
    ...
    m_chunked_data_queue.Push(ChunkedDataQueueEntry{
        std::move(packet), pid, TargetMode::Only, title});
  }
  m_chunked_data_event.Set();
}
...
{
  {
    std::lock_guard lkq(m_crit.chunked_data_queue_write);
    m_chunked_data_queue.Push(ChunkedDataQueueEntry{
        std::move(packet), skip_pid, TargetMode::AllExcept, title});
  }
  m_chunked_data_event.Set();
}
...
bool NetPlayServer::SetupNetSettings()
{
  ...
  Config::AddLayer(ConfigLoaders::GenerateGlobalGameConfigLoader(game->GetGameID(),
      game->GetRevision()));
  Config::AddLayer(ConfigLoaders::GenerateLocalGameConfigLoader(game->GetGameID(),
      game->GetRevision()));
...
std::optional<SaveSyncInfo> NetPlayServer::CollectSaveSyncInfo()
{
                                      ...
                                      sync_info.game->GetPlatform() == DiscIO::Platform::ELFOrDOL))
  {
    ...
    {
      auto file = sync_info.configured_fs->OpenFile(IOS::PID_KERNEL, IOS::PID_KERNEL,

...
bool CBoot::Load_BS2(Core::System& system, const std::string& boot_rom_filename)
{
  ...
  if (data.size() > 0x100)
  {
    ExpansionInterface::CEXIIPL::Descrambler(data.data() + 0x100,
        static_cast<u32>(std::min<size_t>(data.size() - 0x100, 0x1AFE00)));
  }
    ...
    std::vector<DiscIO::Riivolution::Patch> riivolution_patches)
{
  ...
  disc.volume = DiscIO::CreateDisc(DiscIO::DirectoryBlobReader::Create(
      ...
      {
        DiscIO::Riivolution::ApplyPatchesToFiles(riivolution_patches,
            DiscIO::Riivolution::PatchIndex::DolphinSysFiles, fst, nullptr);
      },
      [&](std::vector<DiscIO::FSTBuilderNode>* fst, DiscIO::FSTBuilderNode* dol_node)
      {
        DiscIO::Riivolution::ApplyPatchesToFiles(riivolution_patches,
            DiscIO::Riivolution::PatchIndex::FileSystem, fst, dol_node);
      }));

...
std::unique_ptr<VKTexture> VKTexture::Create(const TextureConfig& tex_config, std::string_view name)
{
  ...
  std::unique_ptr<VKTexture> texture = std::make_unique<VKTexture>(tex_config, alloc, image, name,
...
{
  std::unique_ptr<VKTexture> texture = std::make_unique<VKTexture>(tex_config,
      VmaAllocation(VK_NULL_HANDLE), image, "", layout, ComputeImageLayout::Undefined);
    ...
    const TextureConfig& config)
{
  ...
  std::unique_ptr<VKStagingTexture> staging_tex = std::make_unique<VKStagingTexture>(PrivateTag{},
      type, config, std::move(staging_buffer), linear_image, linear_image_alloc);

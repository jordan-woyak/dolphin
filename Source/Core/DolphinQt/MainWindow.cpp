    ...
    : QMainWindow(nullptr), m_system(system)
{
  ...
  m_config_changed_callback_id = Config::AddConfigChangedCallback([this]
      { QueueOnObject(this, [this] { this->OnHardcoreChanged(); }); });
    ...
    std::unique_ptr<BootSessionData> boot_session_data)
{
  ...
  StartGame(BootParameters::GenerateFromFile(path,
      boot_session_data ? std::move(*boot_session_data) : BootSessionData()));
}

void MainWindow::StartGame(const std::vector<std::string>& paths,
    std::unique_ptr<BootSessionData> boot_session_data)
{
  StartGame(BootParameters::GenerateFromFile(paths,
      boot_session_data ? std::move(*boot_session_data) : BootSessionData()));
}
...
void MainWindow::StateLoad()
{
  ...
  QString path = DolphinFileDialog::getOpenFileName(this, tr("Select a File"), dialog_path,
...
void MainWindow::StateSave()
{
  QString dialog_path = (Config::Get(Config::MAIN_CURRENT_STATE_PATH).empty()) ?
                            QDir::currentPath() :
                            QString::fromStdString(Config::Get(Config::MAIN_CURRENT_STATE_PATH));
  QString path = DolphinFileDialog::getSaveFileName(this, tr("Select a File"), dialog_path,
      tr("All Save States (*.sav *.s##);; All Files (*)"));
  Config::SetBase(Config::MAIN_CURRENT_STATE_PATH, QFileInfo(path).dir().path().toStdString());
  if (!path.isEmpty())
...
bool MainWindow::NetPlayJoin()
{
  ...
  Settings::Instance().ResetNetPlayClient(new NetPlay::NetPlayClient(host_ip, host_port,
      m_netplay_dialog, nickname,
...
bool MainWindow::NetPlayHost(const UICommon::GameFile& game)
{
  ...
  // Create Server
  Settings::Instance().ResetNetPlayServer(new NetPlay::NetPlayServer(host_port, use_upnp,
      m_netplay_dialog,
      NetPlay::NetTraversalConfig{
...
void MainWindow::OnPlayRecording()
{
  ...
  QString dtm_file = DolphinFileDialog::getOpenFileName(this,
      tr("Select the Recording File to Play"), QString(), tr("Dolphin TAS Movies (*.dtm)"));
...
void MainWindow::OnExportRecording()
{
  ...
  QString dtm_file = DolphinFileDialog::getSaveFileName(this, tr("Save Recording File As"),

...
void WiimoteControllersWidget::CreateLayout()
{
  ...
  m_bt_contents = new QStackedWidget;

  m_wiimote_layout->addWidget(m_bt_contents, m_wiimote_layout->rowCount(), 0, 1, -1);

  auto* const emulated_bt_contents = new QWidget;
  auto* const emulated_bt_layout = new QGridLayout{emulated_bt_contents};
  m_bt_contents->addWidget(emulated_bt_contents);
  ...
  auto* const passthru_bt_contents = new QWidget;
  auto* const passthru_bt_layout = new QGridLayout{passthru_bt_contents};
  m_bt_contents->addWidget(passthru_bt_contents);

  int adapter_row = passthru_bt_layout->rowCount();
  passthru_bt_layout->addWidget(m_bluetooth_adapters_label, adapter_row, 1, 1, 1);
  passthru_bt_layout->addWidget(m_bluetooth_adapters, adapter_row, 2, 1, 2);
...
void WiimoteControllersWidget::LoadSettings(Core::State state)
{
  ...
  m_bt_contents->setCurrentIndex(m_wiimote_passthrough->isChecked() ? 1 : 0);

  const bool is_running_gc = running && !Core::System::GetInstance().IsWii();

  // This is not relevant in GC mode.
  m_bt_contents->setEnabled(!is_running_gc);

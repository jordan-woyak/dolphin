...
GameConfigWidget::GameConfigWidget(const UICommon::GameFile& game) : m_game(game)
{
  ...
  m_layer = std::make_unique<Config::Layer>(ConfigLoaders::GenerateLocalGameConfigLoader(m_game_id,
      m_game.GetRevision()));
  m_global_layer =
      std::make_unique<Config::Layer>(ConfigLoaders::GenerateGlobalGameConfigLoader(m_game_id,
          m_game.GetRevision()));
  ...
  connect(tab_widget, &QTabWidget::currentChanged, this,
      [this, editor_index](int index)
      {
        ...
        if (index == editor_index)
        {
          ...
          // There can be multiple ini loaded for a game, only replace the one related to the
          // game ini being edited.
          for (int i = 0; i < m_local_tab->count(); i++)
          {
            if (m_local_tab->tabText(i).toStdString() == m_game_id + ".ini")
            {
              ...
              auto* edit = new GameConfigEdit(nullptr,
                  QString::fromStdString(File::GetUserPath(D_GAMESETTINGS_IDX) + m_game_id +
                                         ".ini"),
                  false);

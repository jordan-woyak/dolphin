...
StackedSettingsWindow::StackedSettingsWindow(QWidget* parent) : QDialog{parent}
{
  ...
  // Most of this stuff is only needed on Windows, but it doesn't seem to hurt elsewhere.
  m_navigation_list->setStyleSheet(
      QStringLiteral("QListWidget { border: 0; background: palette(alternate-base); }"

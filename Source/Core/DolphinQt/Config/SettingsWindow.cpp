...
StackedSettingsWindow::StackedSettingsWindow(QWidget* parent) : QDialog{parent}
{
  ...
  m_navigation_list->setStyleSheet(
      QString::fromUtf8(
          // Maintain consistent highlight color on focus loss.
          "QListWidget::item:selected { background: palette(highlight); "
          "color: palette(highlighted-text); } "
          // Remove border around entire widget and adjust background color.
          "QListWidget { border: 0; background: "
  // FYI: "base" is the window color on Windows and "alternate-base" looks terrible on macOS.
#if defined(_WIN32)
          "palette(alternate-base); "
#else
          "palette(base); "
#endif
          "} QListWidget::item { padding: %1; "
#if defined(_WIN32)
          // For some reason this is needed to fix inconsistent item padding on Windows,
          //  but it unfortunately also breaks hover coloring.
          "border: 0; } "
          // This brings back hover coloring, though not really the right color.

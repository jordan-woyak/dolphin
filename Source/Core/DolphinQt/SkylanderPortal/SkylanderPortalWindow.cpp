...
void SkylanderPortalWindow::SelectCollectionPath()
{
  QString dir = QDir::toNativeSeparators(DolphinFileDialog::getExistingDirectory(this,
      tr("Select Skylander Collection"), m_collection_path));
  if (!dir.isEmpty())
  {
    dir += QDir::separator();
...
void SkylanderPortalWindow::CreateSkylanderAdvanced()
{
  ...
  connect(buttons, &QDialogButtonBox::accepted, this,
      [=, this]
      {
        ...
        QString file_path = DolphinFileDialog::getSaveFileName(this, tr("Create Skylander File"),
            predef_name, tr("Skylander (*.sky);;All Files (*)"));
        if (file_path.isEmpty())
        {
          return;
        }
...
void SkylanderPortalWindow::RefreshList()
{
  ...
  if (m_only_show_collection->isChecked())
  {
    ...
    auto& system = Core::System::GetInstance();
    for (const auto& file :
        collection.entryInfoList(QStringList()
                                 << QStringLiteral("*.sky") << QStringLiteral("*.bin")

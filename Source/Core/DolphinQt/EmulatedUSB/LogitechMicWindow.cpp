...
void LogitechMicWindow::CreateMainWindow()
{
  ...
  m_checkbox_enabled->setChecked(Config::Get(Config::MAIN_EMULATE_LOGITECH_MIC_1));
  ...
  const int volume_modifier = std::clamp<int>(
      Config::Get(Config::MAIN_LOGITECH_MIC_1_VOLUME_MODIFIER), FILTER_MIN, FILTER_MAX);
  ...
  connect(filter_slider, &QSlider::valueChanged, this, [slider_label](int value) {
    Config::SetBaseOrCurrent(Config::MAIN_LOGITECH_MIC_1_VOLUME_MODIFIER, value);
  ...
  auto current_device_id =
      QString::fromStdString(Config::Get(Config::MAIN_LOGITECH_MIC_1_MICROPHONE));

...
GameListModel::GameListModel(QObject* parent) : QAbstractTableModel(parent)
{
  ...
  connect(&Settings::Instance(), &Settings::ThemeChanged,
      [this]
      {
        // Tell the view to repaint. The signal 'dataChanged' also seems like it would work
        // here, but unfortunately it won't cause a repaint until the view is focused.
...
QVariant GameListModel::data(const QModelIndex& index, int role) const
{
  ...
  case Column::Banner:
    if (role == Qt::DecorationRole)
    {
      ...
      banner.setDevicePixelRatio(std::max(static_cast<qreal>(banner.width()) /
                                              GAMECUBE_BANNER_SIZE.width(),

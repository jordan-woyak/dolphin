    ...
    DiscIO::Riivolution::Disc input_disc)
{
  ...
  connect(xml_root_open, &QPushButton::clicked, this,
      ...
      {
        QString dir = QDir::toNativeSeparators(QFileDialog::getExistingDirectory(this,
            tr("Select the Virtual SD Card Root"), xml_root_line_edit->text()));
        if (!dir.isEmpty())
        {
...
RiivolutionBootWidget::LoadConfigXML(const std::string& root_directory)
{
  ...
  return DiscIO::Riivolution::ParseConfigFile(fmt::format("{}/riivolution/config/{}.xml",
      root_directory, m_game_id.substr(0, 4)));
}
...
void RiivolutionBootWidget::SaveConfigXMLs()
{
  ...
  for (const auto& disc : m_discs)
  {
    ...
    for (const auto& section : disc.disc.m_sections)
    {
      for (const auto& option : section.m_options)
      {
        ...
        config_options.emplace_back(DiscIO::Riivolution::ConfigOption{
  ...
  {
    DiscIO::Riivolution::WriteConfigFile(fmt::format("{}/riivolution/config/{}.xml", config.first,
                                             m_game_id.substr(0, 4)),
        config.second);
  }
}
  ...
  for (const auto& disc : m_discs)
  {
    ...
    for (auto& patch : patches)
    {
      patch.m_file_data_loader =
          std::make_shared<DiscIO::Riivolution::FileDataLoaderHostFS>(disc.root,
              disc.disc.m_xml_path, patch.m_root);
    }

...
void MemArena::UnmapFromMemoryRegion(void* view, size_t size)
{
  if (m_memory_functions.m_api_ms_win_core_memory_l1_1_6_handle.IsOpen())
  {
    if (static_cast<PUnmapViewOfFileEx>(m_memory_functions.m_address_UnmapViewOfFileEx)(view,
...
void* LazyMemoryRegion::Create(size_t size)
{
  ...
  {
    void* result =
        static_cast<PMapViewOfFile3>(m_memory_functions.m_address_MapViewOfFile3)(zero_block,
            nullptr, memory + i * BLOCK_SIZE, 0, BLOCK_SIZE, MEM_REPLACE_PLACEHOLDER, PAGE_READONLY,
            nullptr, 0);
    if (!result)
    {
...
void LazyMemoryRegion::Clear()
{
  ...
  for (size_t i = 0; i < m_writable_block_handles.size(); ++i)
  {
    ...
    // map the zero block
    void* map_result =
        static_cast<PMapViewOfFile3>(m_memory_functions.m_address_MapViewOfFile3)(m_zero_block,
            nullptr, memory + i * BLOCK_SIZE, 0, BLOCK_SIZE, MEM_REPLACE_PLACEHOLDER, PAGE_READONLY,
            nullptr, 0);
    if (!map_result)
    {
...
void LazyMemoryRegion::MakeMemoryBlockWritable(size_t block_index)
{
  ...
  // map the new block
  void* map_result =
      static_cast<PMapViewOfFile3>(m_memory_functions.m_address_MapViewOfFile3)(block, nullptr,
          memory + block_index * BLOCK_SIZE, 0, BLOCK_SIZE, MEM_REPLACE_PLACEHOLDER, PAGE_READWRITE,
          nullptr, 0);
  if (!map_result)
  {

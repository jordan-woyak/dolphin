...
void ReshapableInput::SaveConfig(Common::IniFile::Section* section, const std::string& base_name)
{
  ...
  const std::ranges::transform_view scaled_calibration(m_calibration,
      [](ControlState val) { return val * CALIBRATION_CONFIG_SCALE; });
...
ReshapableInput::ReshapeData ReshapableInput::Reshape(ControlState x, ControlState y,
    ControlState modifier, ControlState clamp) const
{
  x -= m_center.x;
  y -= m_center.y;
